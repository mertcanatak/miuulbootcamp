...
df = pd.read_csv("D:\mert python\CSV Files/diabetes.csv")
...
def thresholds(dataframe, col):
    quantile1 = dataframe[col].quantile(0.25)
    quantile3 = dataframe[col].quantile(0.75)
    inter_q = quantile3 - quantile1
    low_l = quantile1 - 1.5 * inter_q
    up_l = quantile3 + 1.5 * inter_q
    return low_l, up_l

def rep_thr(dataframe, col):
    low_l, up_l = thresholds(dataframe,col)
    dataframe.loc[dataframe[col] < low_l, col] = low_l
    dataframe.loc[dataframe[col] > up_l, col] = up_l
    return dataframe
...
df["glikoce_rate"] = ["AZ" if value < 50 else "hatalı" if value == 0 else "çok" for value in df["Glucose"]]

df["glikoce_rate"].isnull().sum()
