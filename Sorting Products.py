...
df = pd.read_csv("D:\mert python\CSV Files\product_sorting.csv")
print(df.shape)
df.head(10)
...
####################
# fit = bunu kullanarak dönüştür diyor minmaxscaler dönüştürdükten sonra bekler
# transform diyoruz ki bu beklettiğin değerleri şuna dönüştür
...
# n yıldızlar
# confidence = hesaplancak z tablo değeri için bir değer elde edebilmek için
...
# fakat bzı sosyal prooflar gözden kaçtı

# 5. ve 1. indextekileri getirdik
df[df["course_name"].index.isin([5, 1])].sort_values("bar_score", ascending=False)
# daha düşük ratingli kursun düşük puan sayıları daha az olduğu için yukarıya çıktı
...
# - Weighted Rating
# - Bayesian Average Rating Score ** olasılıksal değerlerdir
...
df = pd.read_csv("D:\mert python\CSV Files/movies_metadata.csv",
...
# nereden ayırabileceğimize baktık
...
df = pd.read_csv("D:\mert python\CSV Files/imdb_ratings.csv")
df = df.iloc[0:, 1:]
